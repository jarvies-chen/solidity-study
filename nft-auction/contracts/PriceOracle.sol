...
contract PriceOracle {
    ...
    function getLatestPrice(address token) external view returns (int256) {
        // Check if we have a test price set
        if (token != address(0) && testTokenPrices[token] > 0) {
            return int256(testTokenPrices[token]);
        }
    ...
    */
    function getEthUsdPrice() external view returns (int256) {
        // Check if we have a test price set
        if (testEthPrices[block.chainid] > 0) {
            return int256(testEthPrices[block.chainid]);
        }
